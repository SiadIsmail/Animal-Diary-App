...
public class PetConditionService
{
    ...
    /// <summary>Add a condition to a pet (no-op if it already has it, or if empty).</summary>
    public async Task AddAsync(int petId, string conditionId)
    {
        if (string.IsNullOrWhiteSpace(conditionId))
            return;

        var already = await _db.Table<PetCondition>()
            .Where(c => c.PetId == petId && c.ConditionId == conditionId)
            .CountAsync();
        if (already > 0)
            return;

        await _db.InsertAsync(new PetCondition { PetId = petId, ConditionId = conditionId });
    }

    /// <summary>Remove a condition from a pet. Its trackers are handled separately by
    /// the caller (kept or turned off) — this only forgets the condition link.</summary>
